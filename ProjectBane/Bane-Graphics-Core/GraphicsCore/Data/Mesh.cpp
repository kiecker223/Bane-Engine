...
void ProcessMesh(aiMesh* pMesh, std::vector<uint32>& OutIndices, std::vector<FloatInt>& OutVertices)
{
	for (uint32 i = 0; i < pMesh->mNumVertices; i++)
	{
		...
		Mesh::Vertex Vtx = { 
			fvec3(Pos.x, Pos.y, Pos.z), 
			fvec3(Norm.x, Norm.y, Norm.z), 
			fvec3(Binormals.x, Binormals.y, Binormals.z), 
			fvec3(Tangent.x, Tangent.y, Tangent.z), 
			fvec2(UV.x, UV.y) 
		};
...
void Mesh::GenerateUVSphere(uint32 SegmentCount)
{
	...
	for (uint32 y = 0; y < SegmentCount / 2 + 1; y++)
	{
		...
		if (y == 0)
		{
			Vertices.push_back({ TopPoint, normalized(TopPoint), fvec3(), fvec3(), fvec2(0.5f, 1.0f) });
		}
		else if (y == ((SegmentCount / 2)))
		{
			Vertices.push_back({ -TopPoint, normalized(-TopPoint), fvec3(), fvec3(), fvec2(0.5f, 0.0f) });
		}
		else
		{
			for (uint32 i = 0; i < SegmentCount; i++)
			{
				...
				fvec3 Point = (fmat3x3)(matRotY(radians(PercentAroundCrown * 360.f)) * matRotX(radians(PercentDown * 180.f))) * TopPoint;
				Vertices.push_back({ Point, normalized(Point), fvec3(), fvec3(), fvec2(1.0f - PercentAroundCrown, 1.0f - PercentDown) });
 				if (i == SegmentCount - 1)
 				{
 					Point = (fmat3x3)(matRotY(radians(360.f)) * matRotX(radians(PercentDown * 180.f))) * TopPoint;
 					Vertices.push_back({ Point, normalized(Point), fvec3(), fvec3(), fvec2(0.0f, 1.0f - PercentDown) });
 				}
			}
		}
	}
...
{
	fvec3 TopMost = fvec3(0.f, 0.f, 0.5f);
	fvec3 BottomMost = -TopMost;
	...
	for (uint32 y = 0; y < HeightSegments + 1; y++)
	{
		...
		for (uint32 i = 0; i < RadialSegments; i++)
		{
			...
			fvec3 Point = fvec3(0.f, 0.5f, 0.5f);

			fvec3 NewPoint = ((fmat3x3)RotateAround * Point) - fvec3(0.f, 0.f, PercentDown);
