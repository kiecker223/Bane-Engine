...
void Font::LoadFont(const std::string& FontName)
{
	...
	std::string ImageName;
	{
		...
		for (uint32 i = 0; i < 127; i++)
		{
			...
			fvec2 Start((StartX - BearingX) / fImgW, StartY / fImgH);
			fvec2 End(StartX + FontAdvance / fImgW, (StartY + (DimensionY + BearingY)) / fImgH);
			m_Characters[i].Data[0].Position = fvec2(-HalfDimX, -HalfDimY);
			m_Characters[i].Data[0].UV = fvec2(Start.x, End.y);
			m_Characters[i].Data[1].Position = fvec2(HalfDimX, -HalfDimY);
			m_Characters[i].Data[1].UV = fvec2(End.x, End.y);
			m_Characters[i].Data[2].Position = fvec2(HalfDimX, HalfDimY);
			m_Characters[i].Data[2].UV = fvec2(End.x, Start.y);
			m_Characters[i].Data[3].Position = fvec2(-HalfDimX, HalfDimY);
			m_Characters[i].Data[3].UV = fvec2(Start.x, Start.y);
		}
	}
	...
	FontDataStructuredBuff = Device->CreateStructuredBuffer(static_cast<uint32>(sizeof(fvec4) * 2 * m_Characters.size()), reinterpret_cast<uint8*>(m_Characters.data()));
