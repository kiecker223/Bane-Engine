...
public:
	...
	static ILboolean	GetResource(ilImage &, HINSTANCE hInst, ILint ID, ILstring ResourceType);
	static ILboolean	GetResource(ilImage &, HINSTANCE hInst, ILint ID, ILstring ResourceType, ILenum Type);
...
public:
	...
	static const ILstring	GetString(ILenum);
...
{
public:
	static void			Check(void (*Callback)(const ILstring));
	static void			Check(void (*Callback)(ILenum));
	static ILenum		Get(void);
	static const ILstring String(void);
	static const ILstring String(ILenum);
...
ILboolean ilWin32::GetResource(ilImage &Image, HINSTANCE hInst, ILint ID, ILstring ResourceType)
{
	Image.Bind();
	return ilutLoadResource(hInst, ID, ResourceType, IL_TYPE_UNKNOWN);
}

ILboolean ilWin32::GetResource(ilImage &Image, HINSTANCE hInst, ILint ID, ILstring ResourceType, ILenum Type)
{
	Image.Bind();
...
const ILstring ilState::GetString(ILenum StringName)
{
...
// ILERROR
//
void ilError::Check(void (*Callback)(const ILstring))
{
	static ILenum Error;
...
const ILstring ilError::String()
{
	return iluErrorString(ilGetError());
}

const ILstring ilError::String(ILenum Error)
{
